...
class VocabEntry(object):
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_t = pad_sents(word_ids, self['<pad>']) # Jerry: at current step it is only word ID, not yet become embeddings
        sents_var = torch.tensor(sents_t, dtype=torch.long, device=device) # Jerry: at current step it is only word ID orgnized in tensor shape(29 sequence,10 samples), not yet become embeddings
